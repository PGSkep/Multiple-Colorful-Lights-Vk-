...
void Renderer::Render()
{
	...
	// Draw
	{
		...
		{
			0,
			Engine::camera.front.x,
			Engine::camera.front.y,
			Engine::camera.front.z,
		};
	...
	glm::mat4 mat = Engine::camera.GetView();

	//std::cout << Engine::camera.front.x << '	' <<
	//	Engine::camera.front.y << '	' <<
	//	Engine::camera.front.z << '	' << "\n\n" <<
	//	mat[0][0] << '	' <<
	//	mat[1][0] << '	' <<
